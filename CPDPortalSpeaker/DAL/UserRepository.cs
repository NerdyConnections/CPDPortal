...
namespace CPDPortalSpeaker.DAL
{
    public class UserRepository : BaseRepository
    {
        ...
        {
            List<UserModel> userList = null;
            userList = (from ui in Entities.UserInfoes where ui.UserType == 2 || ui.UserType ==3 //speakers: usertype=2  moderator: usertype=3
        ...
        public UserModel GetUserDetails(string username)
        {
           ...
           //for example for bad to bones (program id 3), 2 entries are made  therapeuticID 2 and programid 3  and also therapeuticID 3 and programid 3
            ...
            foreach (var item in ProgramIDList)
            {
                ...
                var objProgram = Entities.Programs.Where(p => p.ProgramID == item).SingleOrDefault();
                spd.ProgramID = item??0;
                ...
                var COISlidesUploaded = Entities.COISlidesUploads.Where(pr => pr.UserID == UserID && pr.ProgramID == item).SingleOrDefault();

                if (COISlidesUploaded!=null)
                {

                    spd.COISlidesUploaded = true;
                    spd.COISlidesExt = Entities.COISlidesUploads.Where(pr => pr.UserID == UserID && pr.ProgramID == item).SingleOrDefault().COISlidesExt;
                }
                else
                    spd.COISlidesUploaded = false;


                var SpeakerChosenProgramDate = Entities.ProgramRequests.Where(pr => pr.ProgramSpeakerID == userid && pr.ProgramID == item && (pr.SpeakerChosenProgramDate ?? false)).FirstOrDefault();
                if (SpeakerChosenProgramDate != null)
                {
                    DisplayProgramMaterial = true;

                }
                var ModeratorChosenProgramDate = Entities.ProgramRequests.Where(pr => pr.ProgramModeratorID == userid && pr.ProgramID == item && (pr.ModeratorChosenProgramDate ?? false)).FirstOrDefault();
                ...
                var SpeakerConfirmedSession = (from pr in Entities.ProgramRequests
                                               ...
                                               where pr.ProgramSpeakerID == userid && pr.ProgramID==item
                ...
                var ModeratorConfirmedSession = (from pr in Entities.ProgramRequests
                                                 ...
                                                 where pr.ProgramModeratorID == userid && pr.ProgramID == item
                                                 select new
                                                 {
                                                     ProgramRequestID = pr.ProgramRequestID,
                                                     ConfirmedDate = (pr.ConfirmedSessionDate.HasValue) ? SqlFunctions.DateName("month", pr.ConfirmedSessionDate) + " " + SqlFunctions.DateName("day", pr.ConfirmedSessionDate) + "," + SqlFunctions.DateName("year", pr.ConfirmedSessionDate) : string.Empty,
